...
function createConquerDOMButton(
    ...
    btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 17.5L3 6V3h3l11.5 11.5"/><line x1="13" y1="19" x2="19" y2="13"/><line x1="16" y1="16" x2="20" y2="20"/><line x1="19" y1="21" x2="21" y2="19"/></svg>`;
    btn.className = `conquer-btn chunk-btn-${chunk.id}`;
    btn.style.top = "4px";
    btn.style.left = "4px";
    btn.title = "Conquer Changes";
    btn.style.backgroundColor = "#eee";
    btn.style.border = "1px solid darkgray";
...
export function CodeEditorWithDiff({
    ...
    // Syncs the vertical scrolling between the left and right panes
    const handleScroll = useCallback(
        (source: "left" | "right") => (e: React.UIEvent<HTMLDivElement>) => {
            const targetRef = source === "left" ? rightPaneRef : leftPaneRef;
            if (!targetRef.current) return;

            const sourceScroll = e.currentTarget.scrollTop;
            if (Math.abs(targetRef.current.scrollTop - sourceScroll) > 1) {
                targetRef.current.scrollTop = sourceScroll;
            }
        },
        [],
    );
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        if (!diffData || !rightPaneRef.current) return;
    ...
    return (
        <>
            ...
            {/* Main wrapper handles overall boundaries, no internal max-widths */}
            <div className="flex flex-row w-full h-[600px] border border-gray-300 rounded shadow-sm bg-white text-sm overflow-hidden">
                <div
                    ref={leftPaneRef}
                    onScroll={handleScroll("left")}
                    className={`h-full overflow-auto ${diff ? "w-1/2 border-r border-gray-200" : "w-full"}`}
                >
                    {/* min-w-max allows CodeMirror to grow horizontally as wide as the longest line */}
                ...
                {diff && (
                    <div
                        ...
                        className="relative w-1/2 h-full bg-gray-50 overflow-auto"
