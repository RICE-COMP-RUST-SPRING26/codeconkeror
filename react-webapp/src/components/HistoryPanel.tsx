...
export default function HistoryPanel({
    ...
    return (
        <div className="h-full">
            {visible && (
                <div className="h-full flex flex-col mt-2 border border-gray-200 rounded bg-white overflow-hidden">
                    {nodes.length === 0 ? (
                        ...
                        <ul className="flex-1 divide-y divide-gray-100 overflow-y-auto">
                            {nodes.map((node) => {
                                ...
                                const author =
                                    (node.metadata?.name as string) ||
                                    (node.metadata?.author as string) ||
                                    "";
